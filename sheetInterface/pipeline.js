...
var sheetPipeline =
  typeof sheetPipeline !== 'undefined'
    ? sheetPipeline
    : (function () {
        if (typeof libKizeo === 'undefined' || libKizeo === null) {
          throw new Error('libKizeo indisponible (sheetInterface/pipeline)');
        }
        ...
        function onOpen() {
          ...
          try {
            sheetTriggers.getStoredTriggerFrequency();
          } catch (e) {
        ...
        function handleProcessSummary(formulaire, context, summary, runStart, ingestFlag) {
          ...
          var lastRunAt = summary.runTimestamp || new Date().toISOString();
          ...
          var metadataUpdateStatus = summary.metadataUpdateStatus || 'SKIPPED';
          if (sheetInterfaceHelpers) {
            try {
              sheetInterfaceHelpers.persistExecutionMetadata({
                sheet: context.sheet,
                configService: sheetConfig.getService(),
          ...
          if (ingestFlag !== 'false' && summary.medias && summary.medias.length && context.config.driveFolderId) {
            try {
              sheetExports.exportMedias(summary.medias, context.config.driveFolderId);
            } catch (mediaError) {
        ...
        function main(options) {
          ...
          var context = sheetConfig.resolveFormulaireContext(spreadsheetBdD);
          if (!context) {
            console.log('main: aucun formulaire configuré pour ce classeur.');
            return;
          }
          var validation = sheetConfig.validateFormConfig(context.config, context.sheet);
          ...
          try {
            ...
            var batchLimit = sheetConfig.getConfiguredBatchLimit(context.config);
