...
TestSuite.addTest('majListeExterne ignore un snapshot incomplet', function (assert) {
...
TestSuite.addTest('handleResponses déclenche majListeExterne avec sheet désactivé', function (assert, withMockedGlobals) {
  const callLog = { service: [], fetch: [] };
  let unreadCallCount = 0;
  const fetchFn = (method, path, payload) => {
    callLog.fetch.push({ method, path, payload });
    ...
    throw new Error('fetch non géré : ' + method + ' ' + path);
  };

  const snapshotService = {
    buildRowSnapshot: () => ({
      existingHeaders: ['champ_a'],
      rowEnCours: ['valeur']
    })
  };
  ...
  withMockedGlobals(
    {
      ExternalListsService: {
        updateFromSnapshot: (formulaire, snapshot, overrides) => {
          callLog.service.push({ formulaire, snapshot, overrides });
          return 'Mise A Jour OK';
        }
      },
      requeteAPIDonnees: fetchFn,
      BigQueryService: {
        getConfig: () => ({ projectId: 'p', datasetId: 'd', location: 'europe-west1' }),
        ensureDataset: () => {},
        ensureRawTable: () => {},
        ensureParentTable: () => {},
        ensureSubTable: () => {},
    ...
    function () {
      ...
      assert.equals(callLog.service.length, 1, 'ExternalListsService doit être appelé une fois');
      const call = callLog.service[0];
      assert.equals(call.formulaire.id, 'FORM-EXT', 'Formulaire transmis à majListeExterne');
      assert.deepEquals(call.snapshot.rowEnCours, ['valeur'], 'Snapshot transmis sans persistance Sheets');
    }
  );
});
...
TestSuite.addTest('parseConfigBooleanFlag et buildExecutionTargets', function (assert) {
  ...
  assert.equals(targets.bigQuery, true, 'BigQuery toujours actif');
  assert.equals(targets.sheet, false, 'synchronisation Sheets toujours désactivée');
  assert.equals(targets.externalLists, true, 'listes externes toujours actives');
});
...
TestSuite.addTest('buildExecutionTargets fallback sans parseConfigBooleanFlag', function (assert, withMockedGlobals) {
  withMockedGlobals(
    ...
    function () {
      ...
      assert.equals(targets.bigQuery, true, 'BigQuery reste activé');
      assert.equals(targets.sheet, false, 'sync Sheets forcée à false');
      assert.equals(targets.externalLists, true, 'listes externes toujours actives');
    }
  );
});
...
TestSuite.addTest('createIngestionServices expose ExternalSnapshot', function (assert, withMockedGlobals) {
  const snapshotStub = {
    marker: 'snapshot'
  };
  withMockedGlobals(
...
TestSuite.addTest('handleResponses ignore le mode legacy Sheets même si flag présent', function (assert, withMockedGlobals) {
  const propertyStore = createMockPropertiesStore({
  ...
  withMockedGlobals(
    ...
    function () {
      ...
      assert.equals(
        usageMarkers.indexOf('legacy_flag_enabled'),
        -1,
        'le flag legacy ne doit plus être activé'
      );
      assert.equals(appendedRows.length, 0, 'aucune écriture Sheet ne doit être effectuée');
    }
  );
});
