...
TestSuite.addTest('majListeExterne ignore un snapshot incomplet', function (assert, withMockedGlobals) {
  const calls = [];
  withMockedGlobals(
    {
      requeteAPIDonnees: (method, path, payload) => {
        ...
        if (path === '/lists/LIST-1') {
          return {
            data: {
              list: {
                items: ['Champ1:Champ1', 'valeur1:valeur1']
              }
            }
          };
        }
    ...
    function () {
      const result = majListeExterne(
        { id: '123', nom: 'Form Test' },
        { existingHeaders: [], rowEnCours: [] }
      );
      assert.equals(result, 'IGNORED', 'Snapshot incomplet doit être ignoré');
    }
  );
  assert.equals(calls.length, 1, 'Une seule requête vers /lists est attendue');
});
