...
TestSuite.addTest('persistLegacySnapshotToSheet écrit une ligne et trace son usage', function (assert, withMockedGlobals) {
  const store = createMockPropertiesStore({
  ...
  const mockSheet = {
    ...
    getRange: function (row, column, numRows, numCols) {
      return {
        getValues: () => {
          if (row === 1 && numRows === 1) {
            return [this.headers.slice(0, numCols)];
          }
          return [[]];
        }
      };
  ...
  let prepareSheetCalls = 0;
  let getColumnIndicesCalls = 0;
  ...
  withMockedGlobals(
    {
      ...
      getFormSheetById: () => mockSheet,
      prepareSheet: () => {
        prepareSheetCalls += 1;
        return 'ok';
      },
      getColumnIndices: (values) => {
        getColumnIndicesCalls += 1;
        return values.map((_, idx) => idx + 1);
      }
    },
    function () {
      const snapshot = {
      ...
      const result = persistLegacySnapshotToSheet({ getId: () => 'SPREADSHEET' }, { id: 'FORM-LEG', nom: 'Legacy' }, snapshot);
      const legacyUsage = store.getAll();
      assert.equals(prepareSheetCalls, 1, 'prepareSheet doit être invoqué');
      assert.equals(getColumnIndicesCalls, 1, 'getColumnIndices doit être invoqué');
      assert.equals(appendedRows.length, 1, 'une ligne doit être ajoutée');
      assert.deepEquals(appendedRows[0], ['A', 'B'], 'données appendRow incorrectes');
      assert.deepEquals(result.rowEnCours, ['A', 'B'], 'snapshot retourné incorrect');
      assert.truthy(
        Object.keys(legacyUsage).some((key) => key.indexOf('LEGACY_USAGE_sheet_append') !== -1),
        'l’usage legacy doit être tracé'
      );
    }
  );
});
  ...
  withMockedGlobals(
    {
      ...
      getFormSheetById: () => sheetMock,
      prepareSheet: () => 'ok',
      getColumnIndices: (values) => values.map((_, idx) => idx + 1),
      recordLegacyUsage: (marker) => {
        if (marker === 'legacy_flag_enabled') {
          legacyFlagRecorded = true;
        }
      }
    },
    function () {
      ...
      assert.truthy(legacyFlagRecorded, 'le flag legacy doit être enregistré');
