...
TestSuite.addTest('bqPrepareParentRow construit les colonnes dynamiques attendues', function (assert) {
  ...
  assert.equals(prepared.row.answer_time_cet, '2024-06-10 10:00:00', 'answer_time CET calculé');
  assert.equals(prepared.row.update_time_cet, '2024-06-10 11:00:00', 'update_time CET calculé');
  assert.equals(
    prepared.row.ingestion_time_cet,
    bqFormatCetDatetime(prepared.row.ingestion_time),
    'ingestion_time CET cohérent'
  );
...
TestSuite.addTest('bqConvertFieldValue prend en charge divers types', function (assert) {
  ...
  const timestampField = bqConvertFieldValue({ type: 'datetime', value: '2025-10-07T09:40:00Z' });
  assert.truthy(timestampField.extraColumns && timestampField.extraColumns.length, 'colonne CET ajoutée');
  assert.equals(timestampField.extraColumns[0].type, 'DATETIME', 'colonne CET en DATETIME');
  assert.equals(timestampField.extraColumns[0].value, '2025-10-07 11:40:00', 'horodatage CET attendu');
});
...
TestSuite.addTest('bqSerializeSubformValue et bqPrepareSubformRows formattent correctement', function (assert) {
  ...
  const parentContext = {
    ...
    answer_time_cet: '2024-01-01 01:00:00',
    update_time: '2024-01-01T01:00:00Z',
    update_time_cet: '2024-01-01 02:00:00',
    ingestion_time: '2024-01-01T02:00:00Z',
    ingestion_time_cet: '2024-01-01 03:00:00'
  };
  ...
  assert.equals(prepared.rows[0].parent_answer_time_cet, '2024-01-01 01:00:00', 'parent CET propagé');
  assert.equals(prepared.rows[0].ingestion_time_cet, '2024-01-01 03:00:00', 'ingestion CET propagé');
});

TestSuite.addTest('bqPrepareMediaRows ajoute les colonnes CET', function (assert) {
  const formulaire = { id: 'FORM-1', nom: 'Formulaire Média' };
  const rows = bqPrepareMediaRows(formulaire, [
    {
      formId: 'FORM-1',
      formName: 'Formulaire Média',
      dataId: 'record-media-1',
      formUniqueId: 'unique-media',
      fieldName: 'Photo',
      fieldType: 'image',
      mediaId: 'media-1',
      fileName: 'photo.jpg',
      driveFileId: 'drive-file-123',
      parentAnswerTime: '2024-03-01T08:00:00Z',
      parentUpdateTime: '2024-03-01T09:00:00Z'
    }
  ]);

  assert.equals(rows.length, 1, 'une ligne média attendue');
  assert.equals(rows[0].parent_answer_time_cet, '2024-03-01 09:00:00', 'CET parent answer');
  assert.equals(rows[0].parent_update_time_cet, '2024-03-01 10:00:00', 'CET parent update');
  assert.equals(
    rows[0].ingestion_time_cet,
    bqFormatCetDatetime(rows[0].ingestion_time),
    'ingestion CET cohérente'
  );
});
...
TestSuite.addTest('bqConvertFieldValue gère les timestamps répétés', function (assert) {
  ...
  assert.truthy(multiValue.extraColumns && multiValue.extraColumns.length, 'colonne CET répétée');
  assert.equals(multiValue.extraColumns[0].mode, 'REPEATED', 'mode CET répété');
  assert.deepEquals(
    multiValue.extraColumns[0].value,
    ['2024-01-01 01:00:00', null],
    'horodatage CET converti'
  );
});
