// SheetAppBindings Version 0.2.0
...
function sheetAppBindingsResolveGlobal(globalObject) {
...
function sheetAppBindingsExpose(bindings, globalObject, options) {
  if (!bindings || typeof bindings !== 'object') {
    throw new Error(SHEET_APP_BINDINGS_LOG_PREFIX + ': expose requiert des bindings valides');
  }

  var target = sheetAppBindingsResolveGlobal(globalObject);
  if (!target || typeof target !== 'object') {
    throw new Error(SHEET_APP_BINDINGS_LOG_PREFIX + ': expose ne parvient pas à résoudre le global');
  }

  var opts = options || {};
  var functionNames =
    Array.isArray(opts.functions) && opts.functions.length ? opts.functions.slice() : Object.keys(bindings);
  var override = opts.override !== false;

  for (var index = 0; index < functionNames.length; index++) {
    var fnName = functionNames[index];
    var fn = bindings[fnName];
    if (typeof fn !== 'function') {
      continue;
    }
    if (!override && typeof target[fnName] === 'function') {
      continue;
    }
    target[fnName] = (function (originalFn) {
      return function () {
        return originalFn.apply(bindings, arguments);
      };
    })(fn);
  }
