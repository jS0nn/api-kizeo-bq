...
function bqEnsureTableColumns(config, tableId, tableResource, requiredColumns, logLabel) {
  const tableRef = `${config.projectId}.${config.datasetId}.${tableId}`;
  const existingFields =
    tableResource && tableResource.schema && Array.isArray(tableResource.schema.fields)
      ? tableResource.schema.fields
      : [];
  const existingNames = new Set(existingFields.map((field) => field.name));
  const missingColumns = requiredColumns.filter((column) => !existingNames.has(column.name));

  if (!missingColumns.length) {
    console.log(`${logLabel} déjà existante: ${tableRef}`);
    return;
  }
  ...
  try {
    ...
    console.log(
      `${logLabel} mise à jour (${tableRef}) -> colonnes ajoutées: ${missingColumns
        .map((column) => column.name)
        .join(', ')}`
    );
  } catch (err) {
    console.log(`${logLabel} patch schema échoué (${tableRef}) -> ${err}`);
    throw err;
  }
}
...
function bqEnsureParentTable(config, tableId) {
  ...
  try {
    ...
    bqEnsureTableColumns(config, tableId, table, BQ_PARENT_BASE_COLUMNS, 'Table parent');
    return;
  } catch (err) {
    if (!bqIsNotFound(err)) {
...
function bqEnsureSubTable(config, tableId) {
  ...
  try {
    const table = BigQuery.Tables.get(config.projectId, config.datasetId, tableId);
    bqEnsureTableColumns(config, tableId, table, BQ_SUBTABLE_BASE_COLUMNS, 'Table fille');
    return;
  } catch (err) {
    if (!bqIsNotFound(err)) {
...
function bqRemoveTable(config, tableId) {
  const result = {
    tableId: tableId || '',
    deleted: false,
    skipped: false,
  ...
  if (!config || !config.projectId || !config.datasetId) {
    ...
    result.message = 'Configuration BigQuery indisponible.';
    return result;
  }
  if (!tableId) {
    result.skipped = true;
    result.reason = 'MISSING_TABLE_ID';
    result.message = 'Identifiant de table manquant.';
    return result;
  }

  const tableRef = `${config.datasetId}.${tableId}`;
  try {
    BigQuery.Tables.remove(config.projectId, config.datasetId, tableId);
    result.deleted = true;
    console.log(`bqRemoveTable: table supprimée ${tableRef}`);
  } catch (err) {
    if (bqIsNotFound(err)) {
      result.skipped = true;
      result.reason = 'TABLE_NOT_FOUND';
      result.message = 'Table déjà absente.';
      console.log(`bqRemoveTable: table absente, suppression ignorée (${tableRef}).`);
      return result;
    }
    result.message = err && err.message ? err.message : String(err);
    console.log(`bqRemoveTable: suppression impossible (${tableRef}) -> ${err}`);
    throw err;
  }
  return result;
}
...
function bqDeleteFormTables(formulaire, options) {
  const config = getBigQueryConfig();
  if (!config) {
    throw new Error('Configuration BigQuery indisponible, suppression annulée.');
  }

  const form = formulaire || {};
  const explicitParent =
    options && options.parentTableId ? String(options.parentTableId || '').trim() : '';
  const parentTableId = explicitParent || bqParentTableId(form) || '';
  ...
  if (!parentTableId) {
    ...
    summary.parent.reason = 'PARENT_ID_MISSING';
    summary.parent.message = 'Impossible de déterminer le nom de la table parent.';
    return summary;
  }

  const subTableIds = bqListFormSubTables(config, form, parentTableId);
  subTableIds.forEach((tableId) => {
    summary.subTables.push(bqRemoveTable(config, tableId));
  });

  const mediaTableId = bqMediaTableId(form);
  if (mediaTableId && mediaTableId !== parentTableId) {
    summary.media = bqRemoveTable(config, mediaTableId);
  } else {
    summary.media = {
      tableId: mediaTableId || '',
      deleted: false,
      skipped: true,
      reason: mediaTableId ? 'MEDIA_MATCHES_PARENT' : 'MEDIA_ID_MISSING',
      message: mediaTableId ? 'Table média identique au parent.' : 'Aucune table média associée.'
    };
  }

  summary.parent = bqRemoveTable(config, parentTableId);
  return summary;
}
