(function (global) {
  /**
   * SheetSnapshot (legacy)
   * ----------------------
   * Conserve les utilitaires historiques nécessaires à la persistance dans Google Sheets
   * (prepareSheet, persistSnapshot, saveDataToSheet).
   * La construction du snapshot est désormais gérée par `FormResponseSnapshot`.
   */
  ...
  function ensureCore() {
    if (typeof global.FormResponseSnapshot !== 'undefined') {
      return global.FormResponseSnapshot;
    }
    if (typeof global.ExternalSnapshot !== 'undefined') {
      return global.ExternalSnapshot;
    }
    throw new Error('FormResponseSnapshot indisponible');
  }
