...
function createIngestionServices(overrides) {
  ...
  const defaultSnapshot = typeof SheetSnapshot !== 'undefined' ? SheetSnapshot : null;
...
function collectResponseArtifacts(records, context) {
  ...
  records.forEach((rep) => {
    ...
    const snapshot = snapshotService && typeof snapshotService.buildRowSnapshot === 'function'
      ? snapshotService.buildRowSnapshot(spreadsheetBdD, formulaire, recordData, medias)
      : buildRowSnapshot(spreadsheetBdD, formulaire, recordData, medias);
    if (snapshot) {
      const persistedSnapshot = legacySheetSyncEnabled
        ? snapshotService && typeof snapshotService.persistSnapshot === 'function'
          ? snapshotService.persistSnapshot(spreadsheetBdD, formulaire, snapshot)
