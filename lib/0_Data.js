  ...
  try {
    ...
    for (let i = 0; i < tabFields[0].length; i++) {
      ...
      if (fieldType === 'photo' || fieldType === 'signature') {
        const mediaLink = gestionChampImage(
          formulaire.id,
          dataResponse.id,
          fieldName,
          fieldValue,
        ...
        values[targetIndex] = mediaLink || '';
        if (mediaLink) {
          medias.push({ dataId: dataResponse.id, name: `${fieldName}_${fieldValue}`, id: mediaLink });
        }
        continue;
      }
