...
function processData(spreadsheetBdD, formulaire, action, nbFormulairesACharger) {
  try {
    ...
    if (handled === null) {
      console.log('processData: handleResponses a retourné null (échec ingestion).');
      return {
        medias: [],
        latestRecord: null,
        lastSnapshot: null,
  ...
  try {
    ...
    if (Array.isArray(finalUnread?.data) && finalUnread.data.length === 0 && lastSnapshot) {
      try {
        ...
        if (listeAjour === null) {
          console.log("majListeExterne: mise à jour échouée, on conserve néanmoins le résumé d'ingestion.");
          metadataUpdateStatus = 'FAILED';
        } else {
