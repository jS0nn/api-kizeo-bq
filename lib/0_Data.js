...
function processData(spreadsheetBdD, formulaire, action, nbFormulairesACharger) {
  try {
    ...
    let alias = formulaire.alias ? formulaire.alias.toString().trim() : '';
    if (!alias && existingConfig && existingConfig.bq_alias) {
      alias = existingConfig.bq_alias.toString().trim();
    }
    alias = alias || formulaire.nom;
    ...
    return {
      ...
      rowCount: handled.rowCount || 0,
      runTimestamp: handled.runTimestamp
    };
  } catch (e) {
  ...
  try {
    ...
    listeReponses.data.forEach((rep) => {
      ...
      if (finalDataId !== undefined && finalDataId !== null && finalDataId !== '') {
        ...
        console.log(`handleResponses: impossible de déterminer l'ID à marquer comme lu (form=${formulaire.id}, record=${recordSummaryId})`);
      }
    });
    ...
    if (processedDataIds.size) {
      markResponsesAsRead(formulaire, action, Array.from(processedDataIds));
    }
    if (latestRecord) {
      console.log(`Dernier enregistrement traité : ${latestRecord.id || latestRecord._id || 'unknown'}`);
    }
...
function markResponsesAsRead(formulaire, action, dataIds) {
  ...
  for (let index = 0; index < dataIds.length; index += chunkSize) {
    ...
    try {
      ...
      const response = requeteAPIDonnees('POST', `/forms/${formulaire.id}/markasreadbyaction/${action}`, payload);
      console.log(
