...
function ensureFormActionCode(spreadsheet, formId) {
  ...
  const newAction = generateActionCode();
  upsertFormConfig(spreadsheet, {
    form_id: id,
    action: newAction
  });
  return newAction;
...
function updateFormRunState(spreadsheet, formId, updates) {
  const payload = Object.assign({}, updates, { form_id: formId });
  upsertFormConfig(spreadsheet, payload);
}
