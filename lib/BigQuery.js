...
function bqIngestParentBatch(formulaire, rows, columnDefs) {
  ...
  try {
    ...
    const dedupeWaitOptions = { retries: 4, delayMs: 5000, minQuietMs: 90000, maxTotalMs: 240000 };
    if (bqWaitForStreamingBufferClear(config, tableId, dedupeWaitOptions)) {
      bqPurgeDuplicateParentRows(config, formulaire, tableId, dedupeWaitOptions);
    } else {
      console.log(`bqIngestParentBatch: purge doublons reportée (streaming buffer actif persistant) -> ${tableId}`);
    }
    ...
    if (rows.length) {
      const postInsertWait = { retries: 2, delayMs: 5000, minQuietMs: 60000, maxTotalMs: 150000 };
      bqPurgeDuplicateParentRows(config, formulaire, tableId, postInsertWait);
    }
  } catch (e) {
    try {
      bqRecordAudit(config, formulaire, {
...
function bqIngestSubTablesBatch(formulaire, tablesDefinition) {
  ...
  tableIds.forEach((tableId) => {
    ...
    try {
      ...
      const dedupeWaitOptions = { retries: 4, delayMs: 5000, minQuietMs: 90000, maxTotalMs: 240000 };
      if (bqWaitForStreamingBufferClear(config, tableId, dedupeWaitOptions)) {
        bqPurgeDuplicateSubTableRows(config, formulaire, tableId, dedupeWaitOptions);
      } else {
        console.log(`bqIngestSubTablesBatch: purge doublons reportée (streaming buffer actif persistant) -> ${tableId}`);
      }
      ...
      if (rowCount) {
        const postInsertWait = { retries: 2, delayMs: 5000, minQuietMs: 60000, maxTotalMs: 150000 };
        bqPurgeDuplicateSubTableRows(config, formulaire, tableId, postInsertWait);
      }
    } catch (e) {
      try {
        bqRecordAudit(config, formulaire, {
...
WHERE STRUCT(form_unique_id, data_id, ingestion_time) IN (
  ...
  try {
    ...
    const isStreamingBufferError =
      e && typeof e.message === 'string' && e.message.toLowerCase().indexOf('streaming buffer') !== -1;
