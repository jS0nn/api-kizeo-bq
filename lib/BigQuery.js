...
function initBigQueryConfig() {
  ...
  return Object.assign({}, BQ_DEFAULT_CONFIG);
}
...
function bqParentTableId(formulaire) {
  const tableIdPart = bqSanitizeTablePart(formulaire?.id, 'form');
  const namePart = bqSanitizeTablePart(formulaire?.nom || formulaire?.alias, 'sheet');
  return bqJoinTableParts([tableIdPart, namePart]);
}
...
function bqIsNotFound(err) {
  ...
  return normalized.indexOf('not found') !== -1 || normalized.indexOf('404') !== -1;
}
