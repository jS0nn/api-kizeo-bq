...
function majListeExterne(formulaire, dataEnCours) {
  try{
    if (!dataEnCours || !Array.isArray(dataEnCours.existingHeaders) || !Array.isArray(dataEnCours.rowEnCours)) {
      console.log('majListeExterne: données snapshot indisponibles, mise à jour ignorée.');
      return 'IGNORED';
    }
    ...
    for (let i = 0; i < listeExterneTotales.lists.length; i++) {
      ...
      if (formulaire.id == liste.idFormulaire) {
        ...
        for (let i = 1; i < splitArray.length; i++) {
          ...
          if (indexVariableEnCours === -1) {
            console.log(`majListeExterne: variable ${variable[0]} introuvable dans le snapshot.`);
            continue;
          }
          let valeurVariableEnCours = dataEnCours.rowEnCours[indexVariableEnCours];
          const safeValue = valeurVariableEnCours === undefined || valeurVariableEnCours === null ? '' : valeurVariableEnCours;

          //remplacement de la donnée de tu Tag X avec la valeur de la donnée présente dans le formulaire
          let replacedData = replaceKizeoData(detailListeExterne.list.items, variable[0], safeValue);
        ...
        if (methode === 'PUT') {
          ...
          requeteAPIDonnees(methode, type, items).data;
        }
      }
    }
