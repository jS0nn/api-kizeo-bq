...
function sheetApplyConfigLayout(sheet, rowCount, options) {
  ...
  if (rowIndexMap && Object.prototype.hasOwnProperty.call(rowIndexMap, 'trigger_frequency')) {
    ...
    if (typeof validationBuilder.requireFormulaSatisfied === 'function') {
      validationBuilder = validationBuilder.requireFormulaSatisfied(formula).setAllowInvalid(false);
    } else if (typeof validationBuilder.requireTextMatchesPattern === 'function') {
      validationBuilder = validationBuilder.requireTextMatchesPattern(triggerPattern).setAllowInvalid(false);
    } else {
      var fallbackList = ['none'].concat(optionKeys);
      validationBuilder = validationBuilder.requireValueInList(fallbackList, true).setAllowInvalid(true);
    }
