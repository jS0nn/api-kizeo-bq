// DriveMediaService Version 0.5.0

function createDriveMediaService(overrides) {
  const driveApp =
    (overrides && overrides.driveApp) ||
    (typeof DriveApp !== 'undefined' ? DriveApp : null);
  const fetchFn =
    (overrides && overrides.fetch) ||
    (typeof requeteAPIDonnees === 'function' ? requeteAPIDonnees : null);
  const handle =
    (overrides && overrides.handleException) ||
    (typeof handleException === 'function' ? handleException : function () {});
  const logger =
  ...
  function saveBlobToFolder(blob, folderId, fileName, options) {
    try {
      const folder = (options && options.folder) || getCachedDriveFolder(folderId);
      if (!folder) {
...
const DRIVE_MEDIA_DEFAULT_SERVICE = (function () {
  try {
    return createDriveMediaService();
  } catch (e) {
    return null;
  }
})();

var DriveMediaService = {
  create: createDriveMediaService,
  getDefault: function () {
    if (DRIVE_MEDIA_DEFAULT_SERVICE) {
      return DRIVE_MEDIA_DEFAULT_SERVICE;
    }
    return createDriveMediaService();
  }
};
